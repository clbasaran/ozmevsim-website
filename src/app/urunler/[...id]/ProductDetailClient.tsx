          ...
          if (typeof initialProduct.features === 'string') {
            ...
            try {
              ...
              if (initialProduct.features && 
                  typeof initialProduct.features === 'string' && 
                  initialProduct.features.length > 2 &&
                  initialProduct.features.startsWith('[') && 
                  initialProduct.features.endsWith(']')) {
                try {
                  ...
                  console.warn('🚨 Slice error on features:', sliceError);
                }
              }
            }
  ...
  useEffect(() => {
    ...
    const loadProduct = async () => {
      try {
        ...
        const specificApiUrl = `/products?id=${productId || id}`;
        
        console.log('🔍 Fetching product by ID from:', specificApiUrl);
        const response = await fetch(specificApiUrl);
        
        if (!response.ok) {
          console.error('❌ Failed to fetch product details:', response.status);
          
          // Fallback: Try to find product in initial data
          if (initialProduct) {
            console.log('✅ Using initial product data as fallback');
        ...
        if (result.success && result.data) {
          console.log('✅ Product details loaded from API:', result.data);
          setProduct(result.data);
        } else {
          console.error('❌ API response indicates failure:', result);
          
          // Fallback: Use initial product if available
          if (initialProduct) {
            console.log('✅ Using initial product data as fallback');
            setProduct(initialProduct);
          } else {
        ...
        setError(`Ürün detayları yüklenirken hata oluştu: ${error instanceof Error ? error.message : 'Bilinmeyen hata'}`);
        
        // Final fallback: Use initial product if available
        if (initialProduct) {
          console.log('✅ Using initial product data as final fallback');
          setProduct(initialProduct);
        }
      } finally {
        setIsLoading(false);
                ...
                try {
                  ...
                  if (typeof rawProductData.features === 'string') {
                    ...
                    try {
                      ...
                      // If it's a stringified array that looks like "[item1, item2]", try to handle it
                      if (rawProductData.features && 
                          typeof rawProductData.features === 'string' && 
                          rawProductData.features.length > 2 &&
                          rawProductData.features.startsWith('[') && 
                          rawProductData.features.endsWith(']')) {
                        try {
                          ...
                          console.warn('🚨 Slice error on rawProductData features:', sliceError);
                        }
                      }
                    }
